...
const authorize = (req, res, next) => {
  ...
  if (req.user.roles.includes('empleado')) {
    ...
    // Verificar si la ruta es de creación o edición
    if (req.method === 'POST' && req.baseUrl === '/inmuebles') {
        console.log("Acceso permitido para crear");
        return next();
    }
    if (req.method === 'PUT' && req.baseUrl === '/inmuebles') {
        console.log("Acceso permitido para actualizar");
        return next();
    }

    // Si no es una ruta de creación o edición, denegar acceso
